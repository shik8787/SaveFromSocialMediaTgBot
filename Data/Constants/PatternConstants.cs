...
public static class PatternConstants
{
    ...
    public const string INSTAGRAM_PHOTO = @"""image_versions2"":\s*{[^}]*""candidates"":\s*\[\s*{[^}]*""url"":\s*""(?<url>https://[^""]+)""";
    public const string INSTAGRAM_DISPLAY_PHOTO = @"""display_url"":\s*""(?<url>https://[^""]+)""";
